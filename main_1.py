...
class MainWindow(QMainWindow):
    ...
    def find_target_clusters(self):
        ...
        centroids = [prop.centroid for prop in self.props]
        ...
        def ellipse_residual_error(params, selected_centroids):
            ...
            ellipse_points = np.array([[a * np.cos(t), b * np.sin(t)] for t in np.linspace(0, 2 * np.pi, 100)])
        ...
        for i, nearest in enumerate(nearest_indices):
            selected_centroids = [centroids[i]] + [centroids[j] for j in nearest]
            ...
            result = minimize(ellipse_residual_error, initial_params, args=(selected_centroids,))
        ...
        for i in target_clusters:
            target_mask[self.labeled_image == self.props[i].label] = 255
